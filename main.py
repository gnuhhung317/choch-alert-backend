...
class ChochAlertSystem:
    ...
    def __init__(self):
        ...
        # DUAL EXCHANGE SETUP
        # ==========================================
        # 1. REALTIME DATA FETCHER (for market analysis)
        #    - Always uses production/realtime data
        #    - Used for CHoCH signal detection
        # 2. DEMO/LIVE EXCHANGE (for order execution)
        #    - Uses testnet (demo) or production (live)
        ...
        self.data_mode = "REALTIME" if config.USE_REALTIME_DATA else "TESTNET"
        logger.info(f"[OK] Market Data Fetcher: {self.data_mode} (for CHoCH detection)")
        ...
        self.demo_exchange = None  # Separate exchange for order execution (demo/live)
    ...
    async def mode_realtime(self):
        ...
        if hasattr(self, '_trading_config'):
            logger.info("\n" + "="*80)
            logger.info("TRADING BOT INITIALIZATION - DUAL EXCHANGE")
            logger.info("="*80)
            ...
            try:
                ...
                # Initialize DEMO/LIVE exchange adapter (for order execution)
                # This is SEPARATE from the realtime data fetcher
                self.demo_exchange = BinanceFuturesAdapter(
                    api_key=self._trading_config['api_key'],
                    secret=self._trading_config['secret'],
                    demo_mode=self._trading_config['demo_mode']  # True=testnet, False=production
                )
                
                # Initialize exchange connection (async)
                await self.demo_exchange.initialize()
                trade_mode = "TESTNET" if self._trading_config['demo_mode'] else "LIVE"
                logger.info(f"✓ Trading Exchange: {trade_mode} (position management)")
                
                # Initialize position manager (uses demo_exchange for orders)
                ...
                logger.info(f"✓ Position Manager: ${position_manager.position_size} @ {position_manager.leverage}x leverage")
                ...
                logger.info(f"✓ Trading Bot: ACTIVE")
                logger.info("="*80 + "\n")
        ...
        logger.info(f"  Market Data: {self.data_mode} (for signal detection)")
        if config.ENABLE_TRADING:
            trade_mode = "TESTNET" if config.DEMO_TRADING else "LIVE"
            logger.info(f"  Trading: {trade_mode} (for order execution)")
        else:
            logger.info(f"  Trading: DISABLED (simulation only)")
        logger.info("=" * 80)
    ...
    async def stop(self):
        ...
        # Close demo/live exchange (for order execution)
        if self.demo_exchange:
            try:
                logger.info("[*] Closing trading exchange connection...")
                await self.demo_exchange.close()
                logger.info("[OK] Trading exchange closed")
            except Exception as e:
                logger.warning(f"Error closing trading exchange: {e}")
        
        # Close market data fetcher (realtime)
        try:
            logger.info("[*] Closing market data fetcher...")
            await self.fetcher.close()
            logger.info("[OK] Market data fetcher closed")
        except Exception as e:
